This repo uses the MAJOR.MINOR.PATCH versioning scheme described here:

  * MAJOR - Incremented at the discretion of the project developers to indicate
    a particularly noteworthy feature or change. An increase in the MAJOR number
    ...
    pool and the feature flags supported by the installed project version.
  ...
  * MINOR - Incremented to indicate new functionality such as a new feature
    ...
    considered to be stable to maximize compatibility between project releases.
...
Two release branches are maintained for the project:

  * project LTS - A designated MAJOR.MINOR release with periodic PATCH
    releases that incorporate important changes backported from newer projects
    ...
    needed. New kernel versions released after the project's LTS release are
    not supported. LTS releases will receive patches for at least 2 years.
    The current LTS release is project 2.2.

  * project current - Tracks the newest MAJOR.MINOR release. This branch
    includes support for the latest project features and recently released
